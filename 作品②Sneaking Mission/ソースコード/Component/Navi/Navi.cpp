...
void Navi::Search(const Math::Vector2& start, const Math::Vector2I& goal)
{
	...
	if (GetNaviState() == NaviState::Search)
	{
		// search中だったら停止するようにするZ
		workThread_.request_stop();
	}

	goal_ = goal;
	auto startPos = static_cast<Math::Vector2I>(start) / tmxObj_->GetTileSize();


	// 探索関数を作業用スレッドで開始する
	workThread_ = std::jthread{
		[this, startPos](std::stop_token token) {
			DebugLog("探索開始");
			std::chrono::time_point now = std::chrono::system_clock::now();
			DijkstraSearch(token,startPos,  goal_); 
			auto result = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now() - now).count() / 1000000000.0;
			DebugLog("終了", result);
		} 
	};
...
void Navi::AstarSearch(std::stop_token token, const Math::Vector2I& start, const Math::Vector2I& goal)
{
	std::lock_guard<std::mutex> lock(mutex_);
	...
	for (int y = 0; y < num.y; y++)
	{
		for (int x = 0; x < num.x; x++)
		{
			...
			{
				nodeVec[y][x].PushNext(&nodeVec[y][x + 1]);
			}
			if (tmxObj_->GetNaviFlag(x - 1, y))
			{
				nodeVec[y][x].PushNext(&nodeVec[y][x - 1]);
			...
			{
				nodeVec[y][x].PushNext(&nodeVec[y + 1][x]);
			}
			if (tmxObj_->GetNaviFlag(x, y - 1))
			{
				nodeVec[y][x].PushNext(&nodeVec[y - 1][x]);
...
void Navi::DijkstraSearch(std::stop_token token, const Math::Vector2I& start, const Math::Vector2I& goal)
{
	std::lock_guard<std::mutex> lock(mutex_);
	...
	// ノードの情報を初期化
	std::vector<std::vector<DijkstraNode>> nodeVec;
	std::list<DijkstraNode*> targetList;
	nodeVec.resize(num.y);
	for (auto& vec : nodeVec)
	{
		vec.resize(num.x);
	...
	for (int y = 0; y < num.y; y++)
	{
		...
		{
			nodeVec[y][x].SetPos(Math::Vector2I{ x,y });
			targetList.emplace_back(&nodeVec[y][x]);

			if (tmxObj_->GetNaviFlag(x + 1, y))
			{
				nodeVec[y][x].PushNextNode(&nodeVec[y][x + 1]);
			}
			if (tmxObj_->GetNaviFlag(x - 1, y))
			{
				nodeVec[y][x].PushNextNode(&nodeVec[y][x - 1]);
			...
			if (tmxObj_->GetNaviFlag(x, y + 1))
			{
				nodeVec[y][x].PushNextNode(&nodeVec[y + 1][x]);
			}
			if (tmxObj_->GetNaviFlag(x, y - 1))
			{
				nodeVec[y][x].PushNextNode(&nodeVec[y - 1][x]);
	...
	// 開始地点のコストをゼロにする
	nodeVec[start.y][start.x].SetTotalCost(0);

	// ソート用ラムダ
	auto sortFunc = [](DijkstraNode* nodeA, DijkstraNode* nodeB)
	{
		return nodeA->GetTotalCost() < nodeB->GetTotalCost();
	};
	...
	// 結果を格納する
	destRoute_.emplace_back(goal);
	for (auto pos = nodeVec[goal.y][goal.x].GetBeforPos(); !(pos == Math::Vector2I{ -1,-1 }); pos = nodeVec[pos.y][pos.x].GetBeforPos())
	{
