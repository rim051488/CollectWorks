...
void Enemy::Draw(const Math::Vector2& offset, DrawMng& drawMng)
{
	...
	if (state_ == EnemyState::Gimic || state_ == EnemyState::MovePatrol)
	{
		auto& result = dynamic_cast<Navi&>(GetComponent(ComponentID::Navi)).GetResult();
		for (auto& pos : result)
		{
			DrawBox((pos.x * 16) + offset.x, (pos.y * 16) + offset.y, ((pos.x * 16) + 16) + offset.x, ((pos.y * 16) + 16) + +offset.y, 0xffffff, true);
		}
	}
